    ...
    <form class="mt-8 space-y-6" on:submit={handleLogin} novalidate>
      {#if error || validationError}
        <div class="bg-elo-danger-100 border-l-4 border-elo-danger-500 text-elo-danger-700 px-4 py-3 rounded">
          <p class="font-medium">{error || validationError}</p>
        </div>
      {/if}
      ...
      <div class="flex items-center justify-center">
        <EloButton disabled={loading} type="submit">
      ...
      <div class="text-center text-sm">
        ...
        <a href="/register" class="font-medium text-elo-primary-600 hover:text-elo-primary-500 underline">
          Sign up
        </a>
      </div>
    </form>
